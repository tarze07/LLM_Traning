...
description: Select the right quantization strategy for LLM deployment based on hardware constraints, quality, and latency
version: 1.0.0
phase: 10
lesson: 11
...
When deploying a language model, use this framework to select the appropriate number format, quantization method, and quality validation strategy.
...
- **Target Latency** (tokens/second, time to first token)
- **Quality Floor** (maximum acceptable perplexity increase, delta from baseline)
...
|--------------|--------|------------|----------------------|
| H100 GPU, max throughput | FP8 E4M3 | Native H100 cast | < 0.1% |
...
- **Tooling:** TensorRT-LLM, FasterTransformer
...
2. **Benchmark check.** Run MMLU (general), GSM8K (math), HumanEval (code). Math and code are the most sensitive to precision loss.

3. **Output comparison.** Generate 100 responses from both the original and quantized model. Use LLM-as-a-judge to compute the win rate. Goal: quantized model wins or ties on > 90% of prompts.

4. **Latency measurement.** Measure tokens per second at batch size 1 and at target batch size. Verify the speedup justifies the quality cost.

5. **Long context test.** If supporting long contexts (> 4K tokens), test at maximum context length. KV cache quantization errors compound with sequence length.
...
Example for Llama 3 70B in INT4, 32K context:
- Weights: 70B * 4 / 8 / 1.07 = 32.6 GB
- KV cache (FP16): 2 * 80 * 8192 * 16 / 8 / 1e9 * 32768 = ~40 GB
- KV cache (FP8): ~20 GB
- Total with FP8 KV: ~55 GB (fits on one 80GB A100)

## Common Mistakes

| Mistake | Why it fails | Fix |
|-------------|------------|-----|
| Quantizing embedding layer to INT4 | The first layer amplifies errors through the whole model | Keep embeddings at FP16 or INT8 |
| Using per-tensor scales for INT4 | One outlier row ruins precision for all rows | Use per-channel or group scales |
| Skipping GPTQ/AWQ calibration | Scale factors are wrong without representative data | Use 128 examples from your domain |
...
| Skipping quality checks | Some models quantize poorly (especially frontier ones) | Always run perplexity + task evals |
